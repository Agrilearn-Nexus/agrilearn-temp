...
export default async function ErrorsPage() {
    ...
    errors.forEach(e => { sourceCounts[e.source] = (sourceCounts[e.source] || 0) + 1; });
        ...
        <div className="min-h-screen bg-gray-50/50 p-6 md:p-8 pt-24 md:pt-28 space-y-8 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">System Error Logs</h1>
                        <p className="text-gray-500 text-sm">Monitor and resolve critical background worker failures.</p>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
                ...
                {errors.length > 0 ? (
                    <ErrorLogTable data={errors} />
                ) : (
                    <div className="p-12 flex flex-col items-center justify-center text-center space-y-3">
                        <div className="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center">
                            <CheckCircle2 className="w-6 h-6 text-green-600" />
                        </div>
                        <h3 className="text-gray-900 font-medium">All Systems Operational</h3>
                        <p className="text-gray-500 text-sm max-w-xs">No errors have been logged recently. Great job!</p>
                    </div>
                )}
            </div>
        </div>
