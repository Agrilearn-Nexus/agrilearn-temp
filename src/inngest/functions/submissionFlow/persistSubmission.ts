...
export const persistSubmission = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const { submissionData, paymentReceipt, paymentReceiptKey, dbId } =
      event.data;
    ...
    const record = await step.run("db-update-relations", async () => {
      ...
      else if (source.includes("PERSON") || submissionData.referredPerson)
        refType = "PERSON";

      if (refType === "PERSON") {
        refName = submissionData.referredPerson?.trim() || "UNKNOWN";
        refDesignation =
          submissionData.referredPersonDesignation?.trim() || "UNKNOWN";
      }
